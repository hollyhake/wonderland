...
```{r}
SlimStampeRData::average_accuracy_participants(data, filepath = "C:/Users/17203/Desktop/Memory/Data/")
SlimStampeRData::average_ROF_participants(data, filepath = "C:/Users/17203/Desktop/Memory/Data/")
SlimStampeRData::average_RT_participants(data, filepath = "C:/Users/17203/Desktop/Memory/Data/")

SlimStampeRData::average_ROF_participants(Pasta, filepath = "C:/Users/17203/Desktop/Memory/Data/")
SlimStampeRData::average_ROF_facts(Pasta, factNames = "factAnswer", filepath = "C:/Users/17203/Desktop/Memory/Data/")

SlimStampeRData::average_accuracy_participants(Pasta, filepath = "C:/Users/17203/Desktop/Memory/Data/")
```

## Functions 

### Average RoF for Every Fact
...
```
### Average RoF for Every Participant in Each Lesson
...
firstsession <- function(df) {
  ...
  graph.clinical=ggplot2::ggplot(data = dat2, ggplot2::aes(x = factor(repetition), y = mean_alpha, group = userId, text=paste0(userId))) +
    ...
    ggplot2::geom_point(data = dat5, alpha = 0.5, size = 3, label = userId, position = ggplot2::position_jitter(w = 0.08, h = 0), ggplot2::aes(colour = factor(userId), fill = factor(userId))) +
...
```{r}
p1 <- average_ROF_participants(Pasta)
p2 <- average_ROF_participants(Swahili1)
p3 <- average_ROF_participants(Flowers)
p4 <- average_ROF_participants(Capitals)

plot_grid(p1, labels=c("Pasta", "Swahili", "Flowers", "Capitals"), ncol = 2, nrow =2)
```
```{r}
p5 <- average_ROF_participants(Birds)
p6 <- average_ROF_participants(News)
p7 <- average_ROF_participants(Flags)
p8 <- average_ROF_participants(Birds)

plot_grid(p5,p6,p7,p8, labels=c("Birds", "News", "Flags", "Birds"), ncol = 2, nrow =2)
...
```{r, Accfact, echo=FALSE, fig.cap="Accuracy of each fact"}

Pasta$userId <- as.character(Pasta$userId)
Pasta$factAnswer <- as.character(Pasta$factAnswer)
Pasta <- Pasta %>%
  mutate(paired= factor(factAnswer)) 

graph.clinical=ggplot(Pasta, aes(x=repetition, y=alpha, text=paste0(factAnswer))) +
  #geom_boxplot(size=0.3) +
  geom_line(aes(group=paired), size=0.2, position=position_dodge(0))+
  geom_point(aes(fill=factAnswer, group=paired), size=1, shape=20, stroke=0, position=position_dodge(0))+
  xlab("Lesson") +
