...
Pasta_RoF <- Pasta_AVG_RoF_Facts %>%
  group_by(factId, repetition) %>%
  summarise(mean_alpha = mean(alpha, na.rm=TRUE), factLabels = unique(factNames)) factLabels = substr(factLabels, 1, 10) %>%
  group_by(factId) %>% 
  filter(repetition == max(repetition)) %>% 
  order(mean_alpha, decreasing = TRUE)

  facts <- sort(unique(Pasta_AVG_RoF_Facts$factId))
  factcolor <- viridis::turbo(length(facts))
  names(factcolor)  <- facts

Pasta_RoF <- Pasta_AVG_RoF_Facts %>%
  group_by(factId, repetition) %>%
  summarise(mean_alpha = mean(alpha, na.rm=TRUE), factLabels = unique(factId))
  Pasta_AVG_RoF_Facts$factLabels = substr(Pasta_AVG_RoF_Facts$factLabels, 1, 10))
  
  dat3 <- group_by(.data = dat2, factId)
  dat4 <- dplyr::filter(.data = dat3, repetition == max(repetition))
  dat5 <- dat4[order(dat4$mean_alpha, decreasing = TRUE),]
